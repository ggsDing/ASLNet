...
class DilatedResBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, d_rate=[1,1]):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=d_rate[0], dilation=d_rate[0], bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=d_rate[1], dilation=d_rate[1], bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.stride = stride
        if inplanes == planes: self.identity = None
        else: self.identity = conv1x1(inplanes, planes)
...
class ASLNet(nn.Module):
    def __init__(self, in_channels=3, num_classes=1, pretrained=True):
        ...
        initialize_weights(self.low, self.DC, self.classifier, self.classifier_aux) #
